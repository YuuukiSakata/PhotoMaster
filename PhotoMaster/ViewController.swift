...
//  PhotoMaster
//
//  Created by Yuki Sakata on 2016/02/12.
...
class ViewController: UIViewController, UINavigationControllerDelegate, UIImagePickerControllerDelegate {
    ...
    func precentPickerController(sourceType: UIImagePickerControllerSourceType){
        ...
        if UIImagePickerController.isSourceTypeAvailable(sourceType) {
            ...
            //UIImagePeckerControllerをインスタンス化
    ...
    func drawText(image: UIImage) -> UIImage {
        ...
        //グラフィッックスコンテキスト生成、編集を開始
        UIGraphicsBeginImageContext(image.size)
        
        //読み込んだ写真を書き出す
        image.drawInRect(CGRectMake(0, 0, image.size.width, image.size.height))
        
        //書き出す位置と大きさと大きさの設定 CGRectMake([左からのx座標]px, [上からのy座標]px, [縦の長さ]px, [横の長さ]px)
        ...
        //グラフィックコンテキストの画像を取得
        let nuwImage = UIGraphicsGetImageFromCurrentImageContext()
    ...
    @IBAction func uploadButtonTapped(sender: UIButton) {
        ...
        guard let _ = photoImageView.image else {
            simpleAlert("画像がありません")
            return
        }
